                    ...
                    for (int ky = 0; ky < layer->kernel_size; ky++) {
                        for (int kx = 0; kx < layer->kernel_size; kx++) {
                            ...
                            if (ix < layer->input_dim && iy < layer->input_dim) {
                                ...
                                size_t input_size = layer->input_dim * layer->input_dim;
                                size_t input_idx = (ic * input_size) + (iy * layer->input_dim + ix);
...
float* conv2d_backward(void *self, float *gradients) {
    ...
    size_t input_dim = layer->input_dim;  // Assuming square input
                    ...
                    for (int ky = 0; ky < layer->kernel_size; ky++) {
                        for (int kx = 0; kx < layer->kernel_size; kx++) {
                            ...
                            if (iy < input_dim && ix < input_dim) {
                                ...
                                // Weight gradients
                                layer->base.weight_gradients[weight_idx] += grad * layer->base.inputs[input_idx];
