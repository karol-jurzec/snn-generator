...
// Jeśli chcesz żeby PruningInfo była kompatybilna z nowym systemem:
typedef struct {
    size_t num_layers;
    ...
    // 🆕 DODATKOWE POLA dla forward compatibility
    bool **inactive_in_channels;     // Forward pruning masks (for future)
    size_t *pruned_in_channels_count; // Forward pruning counts (for future)
...
// Dodaj do istniejącego header file:
